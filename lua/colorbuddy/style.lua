...
local style_add = function(left, right)
    local values = {}
    for index, _ in pairs(left.values) do values[index] = true end
    for index, _ in pairs(right.values) do values[index] = true end
...
local style_sub = function(left, right)
    ...
    for index, _ in pairs(right.values) do values[index] = nil end

    -- if #values == 0 then return styles.none end
    local send_none = true
    for index, _ in pairs(values) do
        if values[index] then send_none = false end
    end
...
setmetatable(read_only_styles, {
    __index = find_style,
    __newindex = function(_, k, _) error('Attempt to modify "styles", a read-only table, with: ' .. k) end,
    __metatable = false,
})
